    ...
    <div
        ...
        <form action="{{route('post.update',[$post->id])}}" method="post">
           ...
           {{method_field('PUT')}}
            <div class="row">
                <div class="col-lg-12">
                    <input class="form-control" autofocus type="text" name="title" placeholder="Titre du post" value="{{$post->title}}">
                    <br>
                    <textarea placeholder="Explication de la note technique" class="form-control" name="content" id="" cols="30" rows="10" required>{{$post->content}}</textarea>
                </div>
            </div>
